...
class M_supplier extends MY_Model{
    ...
    public function dataList(){
		...
		$where = " WHERE supplier_status = 1 AND operator_status = 1";
		...
		$sql = "SELECT
					...
					OP.operator_name
				FROM
					app.t_mtr_supplier SS
					JOIN app.t_mtr_operator OP ON OP.operator_id = SS.operator_id
				$where";
	...
	{
		return $this->db->query("SELECT * FROM app.t_mtr_operator WHERE operator_status = 1")->result();
	}

	public function count_code($code)
	{
		return $this->db->query("SELECT COUNT(*) FROM app.t_mtr_supplier WHERE supplier_code ilike '$code%'")->row();exit;
	}
	...
	{
		return $this->db->query("SELECT * FROM app.t_mtr_supplier WHERE id = $id")->row();
	}
